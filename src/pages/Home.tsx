          ...
          <div className="container mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
              <FeatureCard 
                ...
                title="Easy Booking"
                description="Book desks, meeting rooms, and event spaces with just a few clicks."
              />
              <FeatureCard 
                icon={<MapPin className="h-6 w-6 md:h-8 md:w-8 text-deskhive-orange" />}
                title="Interactive Floor Plans"
                description="Visualize and select workspaces using interactive floor plans."
              />
              <FeatureCard 
                icon={<Users className="h-6 w-6 md:h-8 md:w-8 text-deskhive-orange" />}
                title="Team Collaboration"
                description="Coordinate bookings for team meetings and collaborative sessions."
              />
              <FeatureCard 
                icon={<Clock className="h-6 w-6 md:h-8 md:w-8 text-deskhive-orange" />}
                title="Real-time Availability"
                description="See workspace availability in real-time and avoid booking conflicts."
              />
              <FeatureCard 
                icon={<Check className="h-6 w-6 md:h-8 md:w-8 text-deskhive-orange" />}
                title="Automated Reminders"
                description="Receive automated reminders for upcoming bookings via email."
              />
              <FeatureCard 
                icon={<Calendar className="h-6 w-6 md:h-8 md:w-8 text-deskhive-orange" />}
                title="Calendar Integration"
                description="Sync your bookings with your calendar for better scheduling."
              />
            </div>
          </div>
        </section>
                  ...
                  <div className="aspect-video bg-deskhive-skyblue/30 rounded-lg relative">
                    ...
                    <div className="absolute inset-0 grid grid-cols-6 grid-rows-4 gap-1 opacity-20">
                      {Array.from({ length: 24 }).map((_, i) => (
                        <div key={i} className="border border-deskhive-navy/20 rounded-sm"></div>
                      ))}
                    </div>
        ...
        <section className="bg-deskhive-navy text-white py-12 md:py-20 px-4 md:px-8 lg:px-16">
          <div className="container mx-auto">
            <div className="glass-gradient p-6 md:p-12 text-center rounded-2xl">
              ...
              <p className="text-sm md:text-base lg:text-lg mb-6 md:mb-8 max-w-2xl mx-auto">
                Join hundreds of Nigerian businesses already using DeskHive to streamline their workspace management
              </p>
              <Link to="/register">
